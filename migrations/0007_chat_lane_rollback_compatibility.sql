-- MHO-250 B4: replace schema-6 triggers additively for populated rollbacks.
-- transition_reason is an execution marker, not durable state. Clearing it in
-- the winning transaction prevents restored old source from inheriting it.
DROP TRIGGER IF EXISTS chat_lane_assignment_transition_guard_v2;
DROP TRIGGER IF EXISTS chat_lane_assignment_transition_apply_v2;
UPDATE chat_lane_assignments SET transition_reason=NULL
WHERE transition_reason GLOB 'mho250-v2:*';
...
BEGIN
  ...
  UPDATE chat_lane_assignments SET transition_reason=NULL WHERE assignment_id=NEW.assignment_id;
END;
INSERT OR REPLACE INTO factory_schema_meta(schema_name,schema_version) VALUES ('factory-ledger',7);
